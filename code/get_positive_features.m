...
features_pos = zeros(num_images,dimen);
for i=1:num_images
	img = im2single(imread(strcat([train_path_pos '/' image_files(i).name])));
	HOG = vl_hog(img, feature_params.hog_cell_size);
	features_pos(i*2-1,:) = reshape(HOG,1,dimen);
	flipped = fliplr(img);
	HOG = vl_hog(flipped, feature_params.hog_cell_size);
	features_pos(i*2,:) = reshape(HOG,1,dimen);
end
